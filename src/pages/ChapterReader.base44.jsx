...
import { ChevronLeft, ChevronRight, List, ArrowLeft, Sun, Moon, BookOpen, Type, MessageCircle, Send, Film } from "lucide-react";
...
export default function ChapterReader() {
  ...
  // Check authentication before allowing access to the chapter
  useEffect(() => {
    let mounted = true;
    ...
    base44.auth.me()
      ...
      .catch(() => {
        ...
        // Guest users are not allowed to read chapters
  ...
  const { data: storyCharacters = [] } = useQuery({
    ...
    queryFn: () =>
      base44.entities.VoiceCharacter.filter({
        story_id: storyId
      }),
    enabled: !!storyId && !!user,
  });

  // Increment chapter + story views once per open.
  useEffect(() => {
  ...
  }, [chapterId, chapter, story, storyId, user]);
  ...
  useEffect(() => {
    ...
    const markRead = async () => {
      const existing =
        await base44.entities.ReadingProgress.filter({
    ...
    markRead()
      ...
      .catch(() => {});
  }, [chapterId, user, chapter, storyId, queryClient]);
  ...
  const goToChapter = (ch, dir) => {
    if (isFlipping || !ch) return;
  ...
  // Guest users should never see the chapter
  if (!user) {
    return null;
  }
                    ...
                    <div className="max-w-3xl mx-auto px-4 py-3 space-y-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          {Object.entries(readingThemes).map(
                            ([key, val]) => (
                              <button
                                key={key}
              ...
              <div className="max-w-2xl mx-auto px-5 sm:px-8 py-10">
                ...
                >
                  ...
                  >
                    ...
                    تعليقات الفصل ({chapterComments.length})
                  </button>
