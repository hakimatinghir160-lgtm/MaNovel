...
import { Plus, Image, Video, Music, Maximize2, Minimize2, Type, Save, X, Eye } from "lucide-react";
...
function createMediaBlock(type, url, caption = "") {
  return { id: Date.now() + Math.random(), type, url, caption };
}
...
export default function RichChapterEditor({
  ...
  const [showMediaPicker, setShowMediaPicker] = useState(null);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const filePath = `chapters/${Date.now()}-${safeName}${fileExt ? `.${fileExt}` : ""}`;
      ...
      const mediaBlock = createMediaBlock(type, fileUrl);
      ...
      setBlocks(prev => {
        ...
        if (idx === -1) {
          return [...prev, mediaBlock, textBlock];
        }
  ...
  return (
    ...
    >
      <div
        className={`flex items-center justify-between gap-2 px-4 py-2 border-b ${borderColor} ${toolbarBg}`}
      >
        <div className="flex items-center gap-1">
          ...
          >
            {wordCount} كلمة • {readingTime} دقيقة
          </span>
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          >
            ...
            >
              <SelectTrigger
                className={`h-7 w-[68px] border-none bg-transparent px-1 py-0 text-xs shadow-none focus:ring-0 ${
                  isDark
                    ? "text-white/70"
          ...
          <button
            onClick={() => setDarkWrite(!darkWrite)}
      ...
      <div
        className={`px-6 sm:px-10 pt-6 pb-3 ${
      ...
      >
        <input
          ...
          className={`w-full text-2xl font-heading font-bold bg-transparent border-none outline-none placeholder:opacity-25 ${
        ...
        <div
          className={`mt-4 h-px ${
            isDark ? "bg-white/5" : "bg-border/40"
          }`}
        />
      </div>
              ...
              {block.type === "text" ? (
                <div className="relative">
                  <textarea
                    ...
                    className={`w-full bg-transparent border-none outline-none resize-none leading-loose placeholder:opacity-20 ${
                ...
                <div className="my-4 rounded-xl overflow-hidden border border-border relative group">
                  ...
                  <input
                    ...
                    className={`w-full text-xs px-3 py-2 border-t border-border bg-transparent outline-none ${
                      isDark
                  ...
                  <button
                    ...
                    className="absolute top-2 left-2 w-6 h-6 rounded-full bg-black/50 text-white opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"
                  >
                    <X className="w-3 h-3" />
                  </button>
                </div>
                ...
                <div
                  className={`my-4 rounded-xl p-3 border flex items-center gap-3 ${
                    isDark
                ...
                <div className="relative">
                  <button
                    ...
                    className={`w-6 h-6 rounded-full border flex items-center justify-center transition-colors ${
                      isDark
                        ? "border-white/20 text-white/40 hover:border-white/40 hover:text-white/70 bg-[#0a0a0a]"
                  ...
                  <AnimatePresence>
                    {showMediaPicker === block.id && (
                      <motion.div
                        ...
                        className={`absolute bottom-8 left-1/2 -translate-x-1/2 rounded-2xl border shadow-xl p-2 z-20 flex gap-1 ${
                          isDark
                        ...
                        {[
                          ...
                          ({
                            ...
                            <button
                              ...
                              onClick={() =>
                                type === "text"
                                  ? insertTextAfter(block.id)
                                  : insertMediaAfter(
                                      block.id,
                                      type
                                    )
                              }
                              className={`flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-colors text-xs ${
                                isDark
      ...
      <div
        className={`flex items-center justify-between px-4 py-3 border-t ${borderColor} ${toolbarBg}`}
      >
        <Button
          variant="ghost"
          size="sm"
